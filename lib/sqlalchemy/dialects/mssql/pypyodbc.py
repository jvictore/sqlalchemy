...
.. dialect:: mssql+pypyodbc
    ...
    :url: https://pypi.org/project/pypyodbc/
...
Hostname-based connections are also supported by pypyodbc.  These are often
...
name must be URL encoded which means using plus signs for spaces::
    ...
    engine = create_engine("mssql+pypyodbc://scott:tiger@myhost:port/databasename?driver=ODBC+Driver+17+for+SQL+Server")

The ``driver`` keyword is significant to the pypyodbc dialect and must be
specified in lowercase.

Any other names passed in the query string are passed through in the pypyodbc
...
will be translated to semicolons when the pypyodbc connect string is generated
internally::

    e = create_engine(
        "mssql+pypyodbc://scott:tiger@mssql2017:1433/test?"
...
A pypyodbc connection string can also be sent in pypyodbc's format directly, as
specified in `the pypyodbc documentation
<https://github.com/mkleehammer/pypyodbc/wiki/Connecting-to-databases>`_,
using the parameter ``odbc_connect``.  A :class:`_sa.engine.URL` object
can help make this easier::

    from sqlalchemy.engine import URL
    connection_string = "DRIVER={SQL Server Native Client 10.0};SERVER=dagger;DATABASE=test;UID=user;PWD=password"
    connection_url = URL.create("mssql+pypyodbc", query={"odbc_connect": connection_string})

    engine = create_engine(connection_url)

.. _mssql_pypyodbc_access_tokens:
...
After getting an engine, the credentials need to be sent to ``pypyodbc.connect``
...
database using Azure credentials::
    ...
    connection_string = "mssql+pypyodbc://@my-server.database.windows.net/myDb?driver=ODBC+Driver+17+for+SQL+Server"
...
.. tip::
    ...
    pypyodbc dialect when no username or password is present.  This needs
...
at both the pypyodbc and engine levels::
...
issue <https://github.com/mkleehammer/pypyodbc/issues/835>`_.
...
pypyodbc Pooling / connection close behavior
------------------------------------------

pypyodbc uses internal `pooling
<https://github.com/mkleehammer/pypyodbc/wiki/The-pypyodbc-Module#pooling>`_ by
...
globally at the pypyodbc module level, **before** any connections are made::
    ...
    engine = create_engine("mssql+pypyodbc://user:pass@dsn")
...
.. seealso::
    ...
    `pooling <https://github.com/mkleehammer/pypyodbc/wiki/The-pypyodbc-Module#pooling>`_ -
    in the pypyodbc documentation.

Driver / Unicode Support
-------------------------

pypyodbc works best with Microsoft ODBC drivers, particularly in the area
of Unicode support on both Python 2 and Python 3.

Using the FreeTDS ODBC drivers on Linux or OSX with pypyodbc is **not**
...
pypyodbc support these are recommended.  FreeTDS remains relevant for
...
pypyodbc have been resolved as of SQLAlchemy 2.0.5. See the notes at
:ref:`mssql_rowcount_versioning`.

.. _mssql_pypyodbc_fastexecutemany:

Fast Executemany Mode
---------------------

The pypyodbc driver includes support for a "fast executemany" mode of execution
...
cursor when an executemany call is to be used.   The SQLAlchemy pypyodbc SQL
...
:func:`_sa.create_engine` , when using the **Microsoft ODBC driver only**::
    ...
    engine = create_engine(
        "mssql+pypyodbc://scott:tiger@mssql2017:1433/test?driver=ODBC+Driver+17+for+SQL+Server",
        fast_executemany=True)

.. versionchanged:: 2.0.9 - the ``fast_executemany`` parameter now has its
   intended effect of this pypyodbc feature taking effect for all INSERT
...
.. seealso::
    ...
    `fast executemany <https://github.com/mkleehammer/pypyodbc/wiki/Features-beyond-the-DB-API#fast_executemany>`_
    - on github

.. _mssql_pypyodbc_setinputsizes:

Setinputsizes Support
-----------------------

As of version 2.0, the pypyodbc ``cursor.setinputsizes()`` method is used for
...
.. versionchanged:: 2.0  The mssql+pypyodbc dialect now defaults to using
...
class _ms_binary_pypyodbc:
    """Wraps binary values in dialect-specific Binary wrapper.
    If the value is null, return a pypyodbc-specific BinaryNull
    object to prevent pypyodbc [and FreeTDS] from defaulting binary
...
class _VARBINARY_pypyodbc(_ms_binary_pypyodbc, VARBINARY):
    pass


class _BINARY_pypyodbc(_ms_binary_pypyodbc, BINARY):
    pass


class _String_pypyodbc(sqltypes.String):
...
class _Unicode_pypyodbc(_MSUnicode):
    def get_dbapi_type(self, dbapi):
        if self.length in (None, "max") or self.length >= 2000:
...
class _UnicodeText_pypyodbc(_MSUnicodeText):
    def get_dbapi_type(self, dbapi):
        if self.length in (None, "max") or self.length >= 2000:
            return (dbapi.SQL_WVARCHAR, 0, 0)
        else:
...
class _JSON_pypyodbc(_MSJson):
    def get_dbapi_type(self, dbapi):
        return (dbapi.SQL_WVARCHAR, 0, 0)


class _JSONIndexType_pypyodbc(_MSJsonIndexType):
    def get_dbapi_type(self, dbapi):
        return dbapi.SQL_WVARCHAR


class _JSONPathType_pypyodbc(_MSJsonPathType):
    def get_dbapi_type(self, dbapi):
        return dbapi.SQL_WVARCHAR
...
class MSExecutionContext_pypyodbc(MSExecutionContext):
    ...
    def pre_exec(self):
        ...
        https://code.google.com/p/pypyodbc/wiki/FAQs#How_do_I_retrieve_autogenerated/identity_values?
...
class MSDialect_pypyodbc(pypyodbcConnector, MSDialect):
    ...
    colspecs = util.update_copy(
        MSDialect.colspecs,
        {
            ...
            sqltypes.VARBINARY: _VARBINARY_pypyodbc,
            sqltypes.LargeBinary: _VARBINARY_pypyodbc,
            sqltypes.String: _String_pypyodbc,
            sqltypes.Unicode: _Unicode_pypyodbc,
            sqltypes.UnicodeText: _UnicodeText_pypyodbc,
            sqltypes.JSON: _JSON_pypyodbc,
            sqltypes.JSON.JSONIndexType: _JSONIndexType_pypyodbc,
            sqltypes.JSON.JSONPathType: _JSONPathType_pypyodbc,
    ...
    def _get_server_version_info(self, connection):
        try:
            ...
            # 2008.  Before we had the VARCHAR cast above, pypyodbc would also
            # fail on this query.
